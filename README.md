...
- **Settings Notification Preferences (Feb 27)** — Notification toggles in `SettingsPage` now persist across sessions. Added `preferences?: string` to the `User` interface in `AuthContext.tsx`. On mount the page parses `user.preferences` JSON to restore saved toggles (falls back to defaults if empty/invalid). Save button calls `PUT /api/auth/me` with `preferences: JSON.stringify(notifications)` — reuses the existing endpoint and `User.preferences` Text column; no backend changes required.
