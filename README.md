...
- **Production ML Serving** — `build_manifest.py` generates `models/artifacts/manifest.json` + per-coin preprocessing artifacts (scalers, feature names). `ml_forecast_service.py` fully rewritten to load all 5 model types via manifest discovery, apply coin-specific RobustScaler preprocessing, run inference from parquet feature data, and convert log-return predictions to prices. Backend container mounts `./models` volume and includes TF/LightGBM/scikit-learn dependencies
- **Auth-gated Navigation** — Landing page hero is publicly accessible. Markets, Forecasts, News, and Watchlist require authentication; unauthenticated clicks open the sign-in modal. Lock icons indicate protected pages. ForecastPanel shows real trained ML models (LightGBM, LSTM, Transformer, TCN, DLinear) instead of ARIMA placeholders
